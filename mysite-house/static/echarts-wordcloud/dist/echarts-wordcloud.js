 ...
 */
// Used to handle the problem of not traversal objects such as Date when merge
var BUILTIN_OBJECT = {
  '[object Function]': 1,
  '[object RegExp]': 1,
  '[object Date]': 1,
  ...
  for (var key in source) {
    if (source.hasOwnProperty(key)) {
      ...
      if (isObject(sourceProp) && isObject(targetProp) && !isArray(sourceProp) && !isArray(targetProp) && !isDom(sourceProp) && !isDom(targetProp) && !isBuiltInObject(sourceProp) && !isBuiltInObject(targetProp) && !isPrimitive(sourceProp) && !isPrimitive(targetProp)) {
        // If recursive overlay is required, the merge is called recursively
        merge(targetProp, sourceProp, overwrite);
      } else if (overwrite || !(key in target)) {
        // Otherwise only handle cases where overwrite is true, or the target object does not have this property
        // Note, if the target[key] does not exist, overwrites it directly
